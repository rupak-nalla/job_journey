...
export default function JobTrackingDashboard() {
  ...
  const [interviewStats, setInterviewStats] = useState({ upcoming: 0, completed: 0, total: 0, success_rate: "0%" });
  ...
  const fetchAllData = async () => {
    ...
    setError(null);
    try {
      await Promise.all([fetchStats(), fetchApplications(), fetchInterviews(), fetchInterviewStats()]);
    } catch (err) {
  ...
  const renderInterviews = () => (
    <>
      <div style={S.intStatsGrid}>
        {[
          { label: "Upcoming", value: interviewStats.upcoming || interviews.length, sub: "Next 7 days", icon: "clock", accent: "#4f46e5", iconBg: "#eef2ff", iconColor: "#4f46e5" },
          { label: "Completed", value: interviewStats.completed || 0, sub: "Last 30 days", icon: "check", accent: "#16a34a", iconBg: "#f0fdf4", iconColor: "#16a34a" },
          { label: "Success Rate", value: interviewStats.success_rate || "0%", sub: "Pass rate", icon: "users", accent: "#ca8a04", iconBg: "#fefce8", iconColor: "#ca8a04" },
        ].map((s) => (
